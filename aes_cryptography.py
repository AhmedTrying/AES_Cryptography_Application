...
def generate_aes_key(key_size=256):
    """
    Generate a random AES key of the specified key size.
    
    Parameters:
    - key_size: The size of the key in bits (default is 256).
    
    Returns:
    - aes_key: The generated AES key.
    """
...
def aes_encrypt(plaintext, aes_key):
    """
    Encrypt plaintext using AES with the provided key.
...
def aes_decrypt(ciphertext, aes_key):
    """
    Decrypt ciphertext using AES with the provided key.
    
    Parameters:
    - ciphertext: The encrypted ciphertext to be decrypted.
    - aes_key: The AES key used for decryption.
    
    Returns:
    - plaintext: The decrypted plaintext message.
    """
